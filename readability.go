...
// All the regular expressions in use within readability.
...
var rxNegative = regexp.MustCompile(`(?i)hidden|^hid$| hid$| hid |^hid |banner|combx|comment|com-|contact|foot|footer|footnote|gdpr|masthead|media|meta|outbrain|promo|related|scroll|share|shoutbox|sidebar|skyscraper|sponsor|shopping|tags|tool|widget|anons|stories|recomend`)
...
func (r *Readability) prepArticle(articleContent *html.Node) {
	...
	r.removeNodes(getElementsByTagName(articleContent, "p"), func(p *html.Node) bool {
		...
		innerText := r.getInnerText(p, false)

		return totalCount == 0 && innerText == ""
	})
		...
		for node != nil {
			...
			if r.flags.stripUnlikelys {
				if rxUnlikelyCandidates.MatchString(matchString) &&
					...
					nodeTagName != "body" &&
					nodeTagName != "a" &&
					len(r.getInnerText(node, true)) < 25 {
					node = r.removeAndGetNext(node)
					continue
				}
			}
